...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_main) //içeriklerin görünümünü düzenliyor
    }

    fun ZarAtma(v: View){
        var rastgele = Random.nextInt(1,7) //rasgele sayı
        val viewImage = findViewById(R.id.resim) as ImageView
        val number = findViewById(R.id.numara) as TextView //number'ı TextView componentı olarak oluşturuyor.
        number.text = rastgele.toString() //number'ın text elemanını rastgele'den atıyor.
        if(rastgele==1){
